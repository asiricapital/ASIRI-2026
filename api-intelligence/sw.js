const CACHE_NAME = 'asiri-intelligence-os-v1.3-pages-release';
const APP_SHELL = [
  './', './index.html',
...
async function cacheFirst(request){ const cached=await caches.match(request); if(cached) return cached; try{ const response=await fetch(request,{cache:'no-cache'}); if(response?.ok&&response.type!=='opaque'){ const cache=await caches.open(CACHE_NAME); await cache.put(request,response.clone()); } return response; }catch(_){ return (await caches.match('./index.html'))||Response.error(); } }
self.addEventListener('fetch', event => { if(event.request.method!=='GET') return; const url=new URL(event.request.url); const isLiveData=url.pathname.endsWith('/live-status.json')||url.pathname.endsWith('/market-data.json'); const isFreshShell=event.request.mode==='navigate'||url.pathname.endsWith('/index.html')||url.pathname.endsWith('/catalog.js')||url.pathname.endsWith('/url-search.js')||url.pathname.endsWith('/sw.js'); if(isLiveData||isFreshShell){ event.respondWith(networkFirst(event.request)); return; } event.respondWith(cacheFirst(event.request)); });
