...
function cleanArtistCandidate(value){
  const cleaned=stripDuration(value)
    .replace(STRIP_WORDS,' ')
    .replace(/\b(?:مع|من|في|على|إلى|الى|لي)\b/gi,' ')
    .replace(/[«»"'()\[\]{}:؛;!?؟.]/g,' ')
    .replace(/\s+/g,' ')
    .trim();
  return cleaned;
...
export function extractArtistHints(prompt){
  ...
  for(const segment of source){
    ...
    if(!cleaned)continue;
    for(const part of cleaned.split(/\s+و\s+|[\/&+]/).map(item=>item.trim())){
...
export function parseSessionIntent(prompt,{seedTrack=null,defaultMinutes=60}={}){
  ...
  const searchText=stripDuration(normalizedPrompt)
    .replace(/\b(?:جلسة|شغل(?:ي)?|ابغى|أبغى|ابي|أبي|أريد|اريد|لمدة|مدة|حوالي|تقريباً|تقريبا)\b/gi,' ')
    .replace(/\s+/g,' ')
    .trim();
  return{
